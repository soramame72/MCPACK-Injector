...
static void ListViewUpdateStatus(int index, int status)
{
    ...
    case 4: txt = L"\u30B9\u30AD\u30C3\u30D7"; break;
    default: txt = L"?"; break;
    }

    if (index >= 0 && index < (int)MainWindow::g_entries.size())
    {
        if (!MainWindow::g_entries[index].errorMsg.empty())
        {
            txt += L" (" + MainWindow::g_entries[index].errorMsg + L")";
        }
    }

    ListView_SetItemText(s_hList, index, 2, const_cast<wchar_t*>(txt.c_str()));
}
...
static void AddPackFile(const std::wstring& path)
{
    ...
    if (_wcsicmp(pExt, L".mcpack")    != 0 &&
        ...
        _wcsicmp(pExt, L".mcworld")   != 0 &&
        _wcsicmp(pExt, L".mctemplate")!= 0 &&
                                           ...
                                           WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        HWND hSt5 = CreateWindowExW(0, L"STATIC",
            L"\u30EF\u30FC\u30EB\u30C9\u30C6\u30F3\u30D7\u30EC\u30FC\u30C8\u30D5\u30A9\u30EB\u30C0:",
            WS_CHILD | WS_VISIBLE,
            10, 212, 300, 16, hWnd, NULL, MainWindow::g_hInst, NULL);
        SendMessage(hSt5, WM_SETFONT, (WPARAM)hFont, TRUE);

        s_pState->hEditTpl = CreateWindowExW(WS_EX_CLIENTEDGE,
            L"EDIT", s_pState->tplPath.c_str(),
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | ES_AUTOHSCROLL,
            10, 230, 330, 22, hWnd,
            reinterpret_cast<HMENU>(IDC_EDIT_TPL_PATH),
            MainWindow::g_hInst, NULL);
        SendMessage(s_pState->hEditTpl, WM_SETFONT, (WPARAM)hFont, TRUE);

        HWND hBr5 = CreateWindowExW(0, L"BUTTON",
            L"\u53C2\u7167...",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
            345, 230, 70, 22, hWnd,
            reinterpret_cast<HMENU>(IDC_BTN_TPL_BROWSE),
            MainWindow::g_hInst, NULL);
        SendMessage(hBr5, WM_SETFONT, (WPARAM)hFont, TRUE);
    ...
    case WM_COMMAND:
    {
        ...
        if (id == IDC_OPT_OK)
        {
            ...
            GetWindowTextW(s_pState->hEditTpl, buf, MAX_PATH);
            s_pState->tplPath = buf;
        ...
        else if (id == IDC_BTN_RES_BROWSE || id == IDC_BTN_BEH_BROWSE || id == IDC_BTN_SKIN_BROWSE || id == IDC_BTN_WLD_BROWSE || id == IDC_BTN_TPL_BROWSE)
        {
            BROWSEINFOW bi = {0};
            bi.hwndOwner = hWnd;
            ...
            if (pidl)
            {
                ...
                if (SHGetPathFromIDListW(pidl, szPath))
                {
                    ...
                    else if (id == IDC_BTN_TPL_BROWSE)
                        SetWindowTextW(s_pState->hEditTpl, szPath);
                }
                CoTaskMemFree(pidl);
            }
        }
...
static void ShowOptionsDialog(HWND hParent)
{
    ...
    state.tplPath  = Settings::worldTemplatesPath;
    state.ok       = false;
    state.hEditRes = NULL;
    state.hEditBeh = NULL;
    ...
    if (state.ok)
    {
        ...
        Settings::worldTemplatesPath= state.tplPath;
        Settings::Save();
    }
}
                                         ...
                                         WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        // Title header
        HWND hSt1 = CreateWindowExW(0, L"STATIC",
            L"MCPACK Injector ver0.0.1x168\n"
            L"\u88FD\u4F5C\u8005: soramame72\n"
            L"\u00A92026 soramame72",
...
static void BrowseAndAdd(HWND hWnd)
{
    ...
    ofn.lpstrFilter  =
        L"Minecraft\u30D1\u30C3\u30AF\u30D5\u30A1\u30A4\u30EB"
        L" (*.mcpack;*.mcaddon;*.mcworld;*.mctemplate;*.zip)\0"
        L"*.mcpack;*.mcaddon;*.mcworld;*.mctemplate;*.zip\0"
        L"\u3059\u3079\u3066\u306E\u30D5\u30A1\u30A4\u30EB (*.*)\0*.*\0\0";
                                          ...
                                          WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        HWND hBtnWld = CreateWindowExW(0, L"BUTTON", L"\u30EF\u30FC\u30EB\u30C9\u3068\u3057\u3066\u5165\u308C\u308B",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
            20, 200, 200, 26, hWnd, reinterpret_cast<HMENU>(105), MainWindow::g_hInst, NULL);
        SendMessage(hBtnWld, WM_SETFONT, (WPARAM)hFont, TRUE);

        HWND hBtnTpl = CreateWindowExW(0, L"BUTTON", L"\u30EF\u30FC\u30EB\u30C9\u30C6\u30F3\u30D7\u30EC\u30FC\u30C8\u3068\u3057\u3066\u5165\u308C\u308B",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
            20, 235, 200, 26, hWnd, reinterpret_cast<HMENU>(106), MainWindow::g_hInst, NULL);
        SendMessage(hBtnTpl, WM_SETFONT, (WPARAM)hFont, TRUE);

        HWND hBtnSkip = CreateWindowExW(0, L"BUTTON", L"\u30B9\u30AD\u30C3\u30D7\u3059\u308B",
            WS_CHILD | WS_VISIBLE | WS_TABSTOP | BS_PUSHBUTTON,
            240, 235, 130, 26, hWnd, reinterpret_cast<HMENU>(103), MainWindow::g_hInst, NULL);
    ...
    case WM_COMMAND:
        ...
        {
            if (s_pState) s_pState->selectedType = PACK_WORLD;
            DestroyWindow(hWnd);
        }
        else if (LOWORD(wParam) == 106)
        {
            if (s_pState) s_pState->selectedType = PACK_TEMPLATE;
            DestroyWindow(hWnd);
        }
...
static void StartInstall(HWND hWnd)
{
    ...
    p->worldTemplatesPath= Settings::worldTemplatesPath;
...
static void OnNotify(HWND hWnd, LPARAM lParam)
{
    ...
    if (pnmh->idFrom == IDC_LIST_PACKS && (pnmh->code == NM_RCLICK || pnmh->code == NM_CLICK))
    {
        ...
        if (pnmia->iItem >= 0 && pnmia->iItem < (int)MainWindow::g_entries.size() && pnmia->iSubItem == 1)
        {
            ...
            AppendMenuW(hPopup, MF_STRING, 1007, L"\u30EF\u30FC\u30EB\u30C9\u30C6\u30F3\u30D7\u30EC\u30FC\u30C8");
            ...
            if ((cmd >= 1001 && cmd <= 1007))
            {
                ...
                else if (cmd == 1007) t = PACK_TEMPLATE;
