...
// Copy src folder to dest path (dest must NOT already exist)
static bool RenameCopyToPath(const std::wstring& src, const std::wstring& dest)
{
    // Ensure destParent exists
    wchar_t szParent[MAX_PATH] = {0};
    wcscpy_s(szParent, _countof(szParent), dest.c_str());
    PathRemoveFileSpecW(szParent);
    CreateDirectoryW(szParent, NULL);

    // SHFileOperation rename-copies src -> dest when dest does not exist
    std::vector<wchar_t> fromBuf(src.begin(), src.end());
    fromBuf.push_back(L'\0');
    fromBuf.push_back(L'\0');

    std::vector<wchar_t> toBuf(dest.begin(), dest.end());
    toBuf.push_back(L'\0');
    toBuf.push_back(L'\0');
...
// =======================================================
static int InstallSinglePack(const PackEntry&      entry,
                               ...
                               const std::wstring&   worldTplPath,
                               HWND                  hWnd,
                               ...
                               std::wstring&         outErrorMsg)
{
    ...
    else if (entry.type == PACK_TEMPLATE) destBase = worldTplPath;
    ...
    if (!Run7zFull(sevenZipPath, entry.path, tempDir, hWnd))
    {
        ...
        outErrorMsg = L"7-Zip\u3067\u306E\u5C55\u958B\u306B\u5931\u6557\u3057\u307E\u3057\u305F";
        return 2;
    }
    ...
    bool ok = RenameCopyToPath(tempDir, targetDir);
    RemoveDirInst(tempDir);
    if (!ok) outErrorMsg = L"\u30D5\u30A1\u30A4\u30EB\u306E\u30B3\u30D4\u30FC\u306B\u5931\u6557\u3057\u307E\u3057\u305F";
    return ok ? 0 : 2;
}
...
// Install .mcaddon
// =======================================================
static int InstallAddon(const PackEntry&      entry,
                          const std::wstring&   sevenZipPath,
                          const std::wstring&   resPath,
                          const std::wstring&   behPath,
                          const std::wstring&   skinPath,
                          ...
                          std::wstring&         outErrorMsg)
{
    ...
    if (subs.empty() && PathFileExistsW(rootManifest))
    {
        ...
        std::wstring dest;
        if      (t == PACK_BEHAVIOR) dest = behPath;
        else if (t == PACK_SKIN)     dest = skinPath;
        else                         dest = resPath;
        ...
        if (!skip)
        {
            ...
            if (RenameCopyToPath(tempDir, target))
                anyOk = true;
            else
                outErrorMsg = L"\u30D5\u30A1\u30A4\u30EB\u306E\u30B3\u30D4\u30FC\u306B\u5931\u6557\u3057\u307E\u3057\u305F";
        }
    }
    else
    {
        for (size_t i = 0; i < subs.size(); ++i)
        {
            ...
            // Route sub-pack to the correct destination folder by type
            std::wstring dest;
            if      (t == PACK_BEHAVIOR) dest = behPath;
            else if (t == PACK_SKIN)     dest = skinPath;
            else                         dest = resPath;
            ...
            {
                rollbackList.push_back(target);
                if (RenameCopyToPath(subs[i], target))
                    anyOk = true;
                else
                    outErrorMsg = L"\u30D5\u30A1\u30A4\u30EB\u306E\u30B3\u30D4\u30FC\u306B\u5931\u6557\u3057\u307E\u3057\u305F";
            }
    ...
    RemoveDirInst(tempDir);
    if (!anyOk && outErrorMsg.empty()) outErrorMsg = L"\u30A4\u30F3\u30B9\u30C8\u30FC\u30EB\u3059\u308B\u30D1\u30C3\u30AF\u304C\u3042\u308A\u307E\u305B\u3093\u3067\u3057\u305F";
    return anyOk ? 0 : 2;
}
...
DWORD WINAPI PackInstaller::InstallThread(LPVOID param)
{
    ...
    for (int i = 0; i < total; ++i)
    {
        ...
        {
            res = InstallSinglePack(e,
                                    p->sevenZipPath,
                                    p->resourcePacksPath,
                                    p->behaviorPacksPath,
                                    p->skinPacksPath,
                                    p->worldsPath,
                                    p->worldTemplatesPath,
                                    p->hWnd,
                                    rollbackList,
                                    errorMsg);
        }

        if (res == 0 && !MainWindow::g_cancelRequested)
        {
            ++success;
            e.status = 2; // Success
        }
        else if (res == 1 || MainWindow::g_cancelRequested)
        {
            e.status = 4; // Skipped
        }
        else
        ...
        MainWindow::g_entries[i].errorMsg = errorMsg;
